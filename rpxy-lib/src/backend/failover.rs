...
impl Default for FailoverConfig {
  fn default() -> Self {
    Self {
      ...
      max_retries: 0, // Will be set based on upstream count
    }
  }
}
...
impl FailoverConfig {
  ...
  pub fn new(
    ...
    let default_max_retries = if num_upstreams > 0 {
      num_upstreams - 1
    } else {
      0
    };
...
mod tests {
  ...
  fn test_failover_config_validate() {
    let valid_config = FailoverConfig::new(Some(vec![404, 502, 503]), None, None, 2);
    assert!(valid_config.validate().is_ok());

    let invalid_config = FailoverConfig::new(Some(vec![200, 502]), None, None, 2);
    assert!(invalid_config.validate().is_err());

    let invalid_config2 = FailoverConfig::new(Some(vec![600]), None, None, 2);
